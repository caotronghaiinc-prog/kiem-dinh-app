-- =========================================================================
-- PROMPT-30 (mentor draft): Seed checklist kiểm định
-- "Thiết bị nâng - Bàn nâng hàng"
--
-- Nguồn: mau-bien-ban/KTAT/DU-THAO-QTKD-III.01-Thiet-bi-nang.pdf, Phụ lục 6
-- "BIÊN BẢN KIỂM ĐỊNH BÀN NÂNG HÀNG" (trang 81-87/87), mục 2 "Kiểm tra bên
-- ngoài, thử không tải, thử tải" -- đủ 3 mục A/B/C, 20 hạng mục. Không seed
-- mục "1- Kiểm tra hồ sơ kỹ thuật" (thang kết quả khác) và dòng trống "Các
-- ghi nhận khác" cuối mục C -- theo đúng quy ước chung.
--
-- Dữ liệu has_presence_flag/value_fields dưới đây đã đối chiếu trực tiếp
-- ảnh chụp 7 trang PDF (81-87) thay vì chỉ dựa vào trích xuất văn bản thô.
--
-- ⚠️ KHÁC HẲN mọi loại thiết bị nâng trước đó -- NHIỀU hạng mục có cờ
-- Có/không có hơn hẳn (bàn nâng hàng loại nhỏ/thuỷ lực có thể không có
-- cáp/xích, không có đường ray...):
-- (1) KHÔNG có mục "Buồng/cabin điều khiển" (giống Palăng/Tời).
-- (2) Mục A chỉ 16 hạng mục (không phải 17/18/16 như các loại trước).
-- (3) Hạng mục 7 "Dẫn động cáp/xích" và 8 "Cụm móc cẩu, puly" CÓ cờ
--     Có/không có -- ĐÂY LÀ ĐIỂM KHÁC DUY NHẤT so với MỌI loại thiết bị
--     nâng khác (Cầu trục/Cần trục/Palăng/Tời/Vận thăng đều KHÔNG có cờ ở
--     2 hạng mục này).
-- (4) Hạng mục 11 "Kiểm tra cáp và loại bỏ" và 12 "Các bộ phận cố định
--     cáp" CŨNG có cờ Có/không có -- khác mọi loại trước (chỉ có value
--     field, không có cờ).
-- (5) Hạng mục 13 "Đường ray" có cờ (giống Palăng/Tời, khác Cầu trục/Vận
--     thăng); hạng mục 14 "Cáp điện, tủ điều khiển" KHÔNG có cờ (giống Cầu
--     trục/Vận thăng, khác Palăng/Tời) -- 2 hạng mục liền kề nhau nhưng
--     LỆCH quy tắc so với các cặp trước, đã đối chiếu kỹ 2 lần qua ảnh để
--     chắc chắn không nhầm.
-- (6) Hạng mục thử tải thứ 3 (20) là "Thử thiết bị khống chế quá tải" --
--     giống Cầu trục/Cần trục/Palăng/Tời, KHÔNG PHẢI "Thử bộ hãm an toàn"
--     như Vận thăng nâng hàng.
--
...
-- Bàn nâng hàng'; (cascade xóa luôn items).
-- =========================================================================
...
values (
  ...
  'Dự thảo QTKĐ III.01 - Thiết bị nâng, Phụ lục 6'
);
...
cross join (values
  ...
  (
    ...
    true, '[{"key":"boi_suat","label":"Bội suất cáp/xích nâng tải","unit":null}]'::jsonb
  ),
  (
  ...
  (
    ...
    'TCVN 10837:2015',
    true, '[{"key":"duong_kinh","label":"Đường kính cáp/xích","unit":null}]'::jsonb
  ),
  (
    ...
    true, '[{"key":"loai_co_dinh","label":"Loại cố định cáp","unit":null}]'::jsonb
  ),
  (
...
where t.equipment_type = 'Thiết bị nâng - Bàn nâng hàng';
