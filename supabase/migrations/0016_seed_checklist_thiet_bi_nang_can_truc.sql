-- =========================================================================
-- Seed checklist kiểm định -- "Thiết bị nâng - Cần trục"
--
-- Nguồn: mau-bien-ban/KTAT/DU-THAO-QTKD-III.01-Thiet-bi-nang.pdf, Phụ lục 2
-- "BIÊN BẢN KIỂM ĐỊNH THIẾT BỊ NÂNG KIỂU CẦN" (trang 54-60/87), mục 2
-- "Kiểm tra bên ngoài, thử không tải, thử tải" -- đủ 3 mục A/B/C, 22 hạng
-- mục (không seed mục "1- Kiểm tra hồ sơ kỹ thuật" -- lý do giống hệt
-- migration 0013: thang kết quả khác "Đầy đủ/Không đầy đủ", và không seed
-- dòng trống cuối mục "Các ghi nhận khác").
--
-- Dữ liệu has_presence_flag/value_fields dưới đây đã đối chiếu trực tiếp
-- ảnh chụp các trang 54-60 (không chỉ dựa trích xuất văn bản thô) -- xác
-- nhận cần trục KHÁC cầu trục ở 3 điểm quan trọng, không phải copy nguyên:
--   1. Có thêm hạng mục 14 "Đối trọng và khung đỡ đối trọng" (TCVN
--      5206:1990) mà cầu trục không có -- cầu trục không cần đối trọng.
--   2. Hạng mục "Đường ray" (15) và "Cáp điện, tủ điều khiển" (16) CÓ cờ
--      "Có/không có" ở cần trục, trong khi ở cầu trục (migration 0013,
--      item 14 "Đường ray" / item 15 "Cáp điện, tủ điều khiển") KHÔNG có
--      cờ này -- đúng nguyên văn 2 Phụ lục khác nhau, không phải lỗi soát.
--   3. Mục C (thử tải) phức tạp hơn hẳn: cần trục có "tầm với" thay đổi
--      được (không cố định như khẩu độ cầu trục) nên mỗi phép thử cần 6-8
--      trường giá trị (tầm với nhỏ nhất/lớn nhất, kèm tải trọng sử dụng/
--      thử ở mỗi tầm với) thay vì 2 trường đơn giản như cầu trục.
--
-- ⚠️ Migration này KHÔNG idempotent -- CHỈ chạy 1 lần trên SQL Editor. Nếu
-- lỡ chạy 2 lần: delete from equipment_checklist_templates where
-- equipment_type = 'Thiết bị nâng - Cần trục'; (cascade xóa luôn items).
-- =========================================================================
...
values (
  ...
  'Dự thảo QTKĐ III.01 - Thiết bị nâng, Phụ lục 2'
);
...
cross join (values
  ...
  (
    ...
    'Thiết bị an toàn: Thiết bị khống chế hành trình (nâng hạ, di chuyển) - Bộ khống chế tải trọng nâng',
  ...
  -- Cần trục có "tầm với" thay đổi được (không cố định như khẩu độ cầu
  -- trục) nên mỗi phép thử cần đủ tải trọng ở CẢ tầm với nhỏ nhất lẫn lớn
  -- nhất -- khác hẳn cầu trục chỉ cần 1 cặp tải trọng sử dụng/thử.
  (
    ...
    false,
    '[
      {"key":"do_cao_nang_moc_lon_nhat","label":"Độ cao nâng móc lớn nhất khi kiểm định","unit":"m"},
      {"key":"chieu_dai_can_chinh_lon_nhat","label":"Chiều dài cần chính lớn nhất khi kiểm định","unit":"m"},
      {"key":"tam_voi_nho_nhat","label":"Tầm với nhỏ nhất","unit":"m"},
      {"key":"tai_su_dung_tam_voi_nho_nhat","label":"Tải trọng sử dụng ở tầm với nhỏ nhất","unit":"tấn"},
      {"key":"tai_thu_tam_voi_nho_nhat","label":"Tải trọng thử ở tầm với nhỏ nhất","unit":"tấn"},
      {"key":"tam_voi_lon_nhat","label":"Tầm với lớn nhất","unit":"m"},
      {"key":"tai_su_dung_tam_voi_lon_nhat","label":"Tải trọng sử dụng ở tầm với lớn nhất","unit":"tấn"},
      {"key":"tai_thu_tam_voi_lon_nhat","label":"Tải trọng thử ở tầm với lớn nhất","unit":"tấn"}
    ]'::jsonb
  ),
  (
  ...
  (
    ...
    'Thử thiết bị khống chế quá tải: 110% SWL',
...
where t.equipment_type = 'Thiết bị nâng - Cần trục';
