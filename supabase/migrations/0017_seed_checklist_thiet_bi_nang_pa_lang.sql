-- =========================================================================
-- PROMPT-24 (mentor draft, chờ chạy tay): Seed checklist kiểm định
-- "Thiết bị nâng - Palăng" (gộp chung Palăng điện + Palăng kéo tay, đúng 1
-- mẫu biên bản duy nhất trong nguồn -- không tách 2 loại riêng)
--
-- Nguồn: mau-bien-ban/KTAT/DU-THAO-QTKD-III.01-Thiet-bi-nang.pdf, Phụ lục 3
-- "BIÊN BẢN KIỂM ĐỊNH PA LĂNG ĐIỆN, PA LĂNG KÉO TAY" (trang 61-66/87), mục 2
-- "Kiểm tra bên ngoài, thử không tải, thử tải" -- đủ 3 mục A/B/C, 20 hạng
-- mục (không seed mục "1- Kiểm tra hồ sơ kỹ thuật" đứng trước vì dùng thang
-- kết quả khác "Đầy đủ/Không đầy đủ", và không seed dòng trống "Các ghi
-- nhận khác" cuối mục C -- giống hệt quy ước đã áp dụng ở Cầu trục/Cần
-- trục).
--
-- Dữ liệu has_presence_flag/value_fields dưới đây đã đối chiếu trực tiếp
-- ảnh chụp 6 trang PDF (61-66) thay vì chỉ dựa vào trích xuất văn bản thô.
--
-- Khác Cầu trục (21 hạng mục) ở 2 điểm:
-- (1) KHÔNG có hạng mục "Buồng/cabin điều khiển" (Cầu trục có ở mục A số
--     5) -- palăng không có cabin vận hành, điều khiển bằng hộp bấm/tay
--     kéo -- nên mục A của Palăng chỉ còn 16 hạng mục thay vì 17.
-- (2) Hạng mục "Đường ray" (13) và "Cáp điện, tủ điều khiển" (14) CÓ cờ
--     "Có/không có" (has_presence_flag=true) -- Cầu trục 2 hạng mục tương
--     ứng (14, 15) KHÔNG có cờ này. Giống đúng khác biệt đã ghi nhận ở Cần
--     trục (migration 0016).
--
-- ⚠️ Migration này KHÔNG idempotent (không có on conflict do nothing) --
-- CHỈ chạy 1 lần trên SQL Editor. Nếu lỡ chạy 2 lần, xóa hết rồi chạy lại:
-- delete from equipment_checklist_templates where
-- equipment_type = 'Thiết bị nâng - Palăng'; (cascade xóa luôn items).
-- =========================================================================
...
values (
  ...
  'Dự thảo QTKĐ III.01 - Thiết bị nâng, Phụ lục 3'
);
...
where t.equipment_type = 'Thiết bị nâng - Palăng';
