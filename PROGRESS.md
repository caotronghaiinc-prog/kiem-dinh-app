...
**Tuần hiện tại:** Tuần 7-8 — Deploy & Onboard  
...
- [ ] **[PROMPT-17]** Hướng dẫn sử dụng cho team + Onboard người dùng
...
- **⚠️ [PROMPT-16] Cần bạn tự xác nhận qua Vercel Dashboard (không có quyền truy cập trực tiếp)**: 4 biến bắt buộc phải có ở **Production environment** (không chỉ Preview/Development): `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `OPENAI_API_KEY`. Xác nhận **KHÔNG có** `E2E_TEST_LOGIN_SECRET` trên Vercel (chỉ tồn tại `.env.local`, đã note từ PROMPT-10) — nếu lỡ có thì `/api/test-login` (đã xác nhận 404 trên production hiện tại) có thể bị khai thác.
- **⚠️ [PROMPT-16] Supabase free tier KHÔNG có automatic backup** (xác nhận qua docs chính thức Supabase, không phải suy đoán) — daily backup chỉ có từ gói Pro trở lên (Free tier chỉ có gợi ý tự `supabase db dump` định kỳ). Đây là rủi ro thật cho giai đoạn "dữ liệu thật" sắp tới (hiện tại còn ít dữ liệu, dùng thử nội bộ nên rủi ro thấp) — **cần bạn quyết định**: chấp nhận rủi ro tạm thời, tự thiết lập backup thủ công định kỳ (`supabase db dump`), hay nâng Supabase lên Pro (lưu ý: đây là gói Supabase, KHÁC với Vercel Pro đã chốt không nâng trong PROMPT này). Xem tại Supabase Dashboard → Database → Backups.
- **⚠️ [PROMPT-16] Supabase Auth "Site URL" vẫn đang trỏ về `http://localhost:3000`**, chưa cập nhật sang domain production — phát hiện khi tạo thử magic link test (link tự động fallback về localhost thay vì `kiem-dinh-app.vercel.app`). Đăng nhập thật (email+mật khẩu) KHÔNG bị ảnh hưởng (không dùng redirect URL), nhưng **"Quên mật khẩu"/magic-link sau này sẽ gửi link hỏng** nếu không sửa. **Cần bạn tự vào**: Supabase Dashboard → Authentication → URL Configuration → đổi Site URL + thêm `https://kiem-dinh-app.vercel.app/**` vào Redirect URLs.
