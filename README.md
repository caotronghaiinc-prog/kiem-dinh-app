...
## 13. Kiểm tra trùng khách hàng (tax_code chặn cứng, tên cảnh báo mềm)
...
- Toàn bộ luồng thêm/sửa khách hàng (PROMPT-05), trang chi tiết (PROMPT-06), đổi quyền inspector cho `customers` (mục 11), danh sách thiết bị (mục 12), và kiểm tra trùng khách hàng (mục 13) cũng chưa tự test được bằng dữ liệu thật vì lý do trên — cần xác nhận qua Preview URL theo các checklist tương ứng.
...
- **Branch `claude/customers-duplicate-check` (mục 13) tách ra từ `master`**, chưa gồm code của PROMPT-08 (form thiết bị, đang ở nhánh `claude/prompt-08-equipment-form` riêng chưa merge) — 2 nhánh độc lập, không phụ thuộc nhau, có thể merge theo thứ tự bất kỳ.
